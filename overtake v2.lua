...
function script.update(dt)
    ...
    if timePassed == 0 then
        ...
        --addMessage('ReMade ByRespect23/ACDC', 2)
        addMessage('CTRL + D -> On/Off', -1)
        ...
        if dangerouslySlowTimer > 3 then
            ...
            if dangerouslySlowTimer < 3 then
                if speedMessageTimer > 5 and not timePassed == 0 then
                    addMessage('3s até perderes o score palhaço!', -1)
                    speedMessageTimer = 0
                end
            end

            if dangerouslySlowTimer == 0 and not timePassed == 0 then
                addMessage('ACELERA POKEMON!', -1)
            end
        end
...
function script.drawUI()
    ...
    if UIToggle then
        ...
        ui.textColored('yuzigang 🩸', rgbm(1, 0, 0, 1)) -- Green text
        ui.popFont()
        ui.popStyleVar()

        ui.pushFont(ui.Font.Huge)
        ui.textColored('PB: ' .. driftPB .. ' pts', rgbm(0, 1, 0, 1)) -- Green text
        ui.popFont()
        ui.beginRotation()
        ui.pushFont(ui.Font.Huge)
        ui.textColored(DriftPoints .. ' pts', rgbm(0, 1, 0, 1)) -- Green text
        ui.sameLine(0, 40)
        ...
        for i = 1, #messages do
            ...
            ui.textColored(m.text, m.mood == 1 and rgbm(0, 1, 0, f)
                or m.mood == -1 and rgbm(1, 0, 0, f) or m.mood == 2 and rgbm(100, 84, 0, f) or rgbm(0, 1, 0, f)) -- Green text
        end
